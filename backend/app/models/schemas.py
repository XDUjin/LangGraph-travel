...
class Attraction(BaseModel):
    ...
    website: Optional[str] = Field(default="", description = "网址")
...
class RAGQueryRequest(BaseModel):
    """RAG 问答请求"""
    question: str = Field(..., description="用户问题", example="北京有哪些必去的景点？")
...
class RAGQueryResponse(BaseModel):
    ...
    success: bool = Field(..., description="是否成功")
    message: str = Field(default="", description="消息")
    answer: Optional[str] = Field(default=None, description="AI 生成的回答")
    sources: List[RAGSource] = Field(default=[], description="参考来源")
    intent: str = Field(default="travel_rag", description="识别的意图: travel_rag / general")


class RAGStatusResponse(BaseModel):
    """RAG 服务状态响应"""
    status: str = Field(..., description="服务状态")
    document_count: int = Field(default=0, description="已索引文档块数量")
    chromadb_path: str = Field(default="", description="ChromaDB 存储路径")
    collection_name: str = Field(default="", description="集合名称")
    embedding_model: str = Field(default="", description="Embedding 模型名")
    knowledge_base_path: str = Field(default="", description="知识库路径")
